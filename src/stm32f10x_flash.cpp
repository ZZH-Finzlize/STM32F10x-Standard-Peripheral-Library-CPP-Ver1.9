...
@code
 ...
 The table below presents the list of available functions depending on the used STM32F10x devices.
   ...
   ***************************************************
   ...
   |----------------------------------------------------------------------------------------------------------------------------------|
   |GetReadOutProtectionStatus    |    Yes     |      Yes      | No change                                                      |
   |----------------------------------------------------------------------------------------------------------------------------------|
   ...
   |----------------------------------------------------------------------------------------------------------------------------------|
   |WaitForLastOperation          |    Yes     |      Yes      | - Wait for Bank1 last operation (for all devices)              |
   |                                    |            |               |   equivalent to: WaitForLastBank1Operation function      |
   +----------------------------------------------------------------------------------------------------------------------------------+
   ...
   ************************************************************************************************************************
   ...
   |----------------------------------------------------------------------------------------------------------------------------------|
   | WaitForLastBank1Operation    |    Yes     |      Yes      | - Wait for Bank1 last operation                                |
   +----------------------------------------------------------------------------------------------------------------------------------+
...
FLASH_TypeDef* FLASH::GetPeriphPointer(void)
{
	return _FLASH;
}
...
/**
  ...
  * @retval None
  */
void FLASH::SetLatency(uint32_t FLASH_Latency)
{
	uint32_t tmpreg = 0;
...
/**
  ...
  * @retval None
  */
void FLASH::HalfCycleAccessCmd(uint32_t FLASH_HalfCycleAccess)
{
	/* Check the parameters */
	assert_param(IS_FLASH_HALFCYCLEACCESS_STATE(FLASH_HalfCycleAccess));

	/* Enable or disable the Half cycle access */
	_FLASH->ACR &= ACR_HLFCYA_Mask;
	_FLASH->ACR |= FLASH_HalfCycleAccess;
}
...
/**
  ...
  * @retval None
  */
void FLASH::PrefetchBufferCmd(uint32_t FLASH_PrefetchBuffer)
{
	/* Check the parameters */
	assert_param(IS_FLASH_PREFETCHBUFFER_STATE(FLASH_PrefetchBuffer));

	/* Enable or disable the Prefetch Buffer */
	_FLASH->ACR &= ACR_PRFTBE_Mask;
	_FLASH->ACR |= FLASH_PrefetchBuffer;
}
...
void FLASH::Unlock(void)
{
	...
	_FLASH->KEYR = FLASH_KEY1;
	_FLASH->KEYR = FLASH_KEY2;

#ifdef STM32F10X_XL
	/* Authorize the FPEC of Bank2 Access */
	_FLASH->KEYR2 = FLASH_KEY1;
	_FLASH->KEYR2 = FLASH_KEY2;
#endif /* STM32F10X_XL */
}
/**
...
void FLASH::Lock(void)
{
	...
	_FLASH->CR |= CR_LOCK_Set;

#ifdef STM32F10X_XL
	/* Set the Lock Bit to lock the FPEC and the CR of  Bank2 */
	_FLASH->CR2 |= CR_LOCK_Set;
#endif /* STM32F10X_XL */
}
...
/**
  ...
  * @retval FLASH Status: The returned value can be: FLASH::BUSY, FLASH::ERROR_PG,
  *         FLASH::ERROR_WRP, FLASH::COMPLETE or FLASH::TIMEOUT.
  */
FLASH::Status FLASH::ErasePage(uint32_t Page_Address)
{
	...
	if(Page_Address < FLASH_BANK1_END_ADDRESS)
	{
		...
		status = WaitForLastBank1Operation(EraseTimeout);
		if(status == FLASH::COMPLETE)
		{
			/* if the previous operation is completed, proceed to erase the page */
			_FLASH->CR|= CR_PER_Set;
			_FLASH->AR = Page_Address;
			_FLASH->CR|= CR_STRT_Set;

			/* Wait for last operation to be completed */
			status = WaitForLastBank1Operation(EraseTimeout);

			/* Disable the PER Bit */
			_FLASH->CR &= CR_PER_Reset;
		}
	}
	else
	{
		...
		{
			/* if the previous operation is completed, proceed to erase the page */
			_FLASH->CR2|= CR_PER_Set;
			_FLASH->AR2 = Page_Address;
			_FLASH->CR2|= CR_STRT_Set;

			/* Wait for last operation to be completed */
			status = FLASH_WaitForLastBank2Operation(EraseTimeout);

			/* Disable the PER Bit */
			_FLASH->CR2 &= CR_PER_Reset;
		}
	}
#else
	/* Wait for last operation to be completed */
...
/**
  ...
  * @note   This function can be used for all STM32F10x devices.
  * @param  None
  * @retval FLASH Status: The returned value can be: FLASH::ERROR_PG,
  *         FLASH::ERROR_WRP, FLASH::COMPLETE or FLASH::TIMEOUT.
  */
FLASH::Status FLASH::EraseAllPages(void)
{
	...
	if(status == FLASH::COMPLETE)
	{
		...
		/* Disable the MER Bit */
		_FLASH->CR &= CR_MER_Reset;
	}
	if(status == FLASH::COMPLETE)
	{
		/* if the previous operation is completed, proceed to erase all pages */
		_FLASH->CR2 |= CR_MER_Set;
		_FLASH->CR2 |= CR_STRT_Set;
...
/**
  ...
  *         FLASH::ERROR_WRP, FLASH::COMPLETE or FLASH::TIMEOUT.
  */
FLASH::Status FLASH::EraseAllBank1Pages(void)
{
	FLASH::Status status = FLASH::COMPLETE;
	/* Wait for last operation to be completed */
	status = WaitForLastBank1Operation(EraseTimeout);
...
/**
  ...
  *         FLASH::ERROR_WRP, FLASH::COMPLETE or FLASH::TIMEOUT.
  */
FLASH::Status FLASH::EraseAllBank2Pages(void)
{
	FLASH::Status status = FLASH::COMPLETE;
	/* Wait for last operation to be completed */
	status = FLASH_WaitForLastBank2Operation(EraseTimeout);
...
FLASH::Status FLASH::EraseOptionBytes(void)
{
	...
	if(status == FLASH::COMPLETE)
	{
		...
		_FLASH->OPTKEYR = FLASH_KEY1;
		_FLASH->OPTKEYR = FLASH_KEY2;

		/* if the previous operation is completed, proceed to erase the option bytes */
		_FLASH->CR |= CR_OPTER_Set;
		_FLASH->CR |= CR_STRT_Set;
		/* Wait for last operation to be completed */
		status = WaitForLastOperation(EraseTimeout);
		...
		if(status == FLASH::COMPLETE)
		{
			...
			_FLASH->CR &= CR_OPTER_Reset;

			/* Enable the Option Bytes Programming operation */
			_FLASH->CR |= CR_OPTPG_Set;
...
/**
  ...
  *         FLASH::ERROR_WRP, FLASH::COMPLETE or FLASH::TIMEOUT.
  */
FLASH::Status FLASH::ProgramWord(uint32_t Address, uint32_t Data)
{
	FLASH::Status status = FLASH::COMPLETE;
	__IO uint32_t tmp = 0;
	...
	{
		/* Wait for last operation to be completed */
		status = WaitForLastBank1Operation(ProgramTimeout);
		if(status == FLASH::COMPLETE)
		{
...
/**
  ...
  *         FLASH::ERROR_WRP, FLASH::COMPLETE or FLASH::TIMEOUT.
  */
FLASH::Status FLASH::ProgramHalfWord(uint32_t Address, uint16_t Data)
{
	FLASH::Status status = FLASH::COMPLETE;
	/* Check the parameters */
	assert_param(IS_FLASH_ADDRESS(Address));
...
/**
  ...
  *         FLASH::ERROR_WRP, FLASH::COMPLETE or FLASH::TIMEOUT.
  */
FLASH::Status FLASH::ProgramOptionByteData(uint32_t Address, uint8_t Data)
{
	FLASH::Status status = FLASH::COMPLETE;
	/* Check the parameters */
...
/**
  ...
  *         FLASH::ERROR_WRP, FLASH::COMPLETE or FLASH::TIMEOUT.
  */
FLASH::Status FLASH::EnableWriteProtection(uint32_t FLASH_Pages)
{
	...
	if(status == FLASH::COMPLETE)
	{
		...
		if((status == FLASH::COMPLETE) && (WRP1_Data != 0xFF))
		{
			OB->WRP1 = WRP1_Data;
		...
		if((status == FLASH::COMPLETE) && (WRP2_Data != 0xFF))
		{
			OB->WRP2 = WRP2_Data;
		...
		if((status == FLASH::COMPLETE)&& (WRP3_Data != 0xFF))
		{
			OB->WRP3 = WRP3_Data;
...
/**
  ...
  *         FLASH::ERROR_WRP, FLASH::COMPLETE or FLASH::TIMEOUT.
  */
FLASH::Status FLASH::ReadOutProtection(FunctionalState NewState)
{
	FLASH::Status status = FLASH::COMPLETE;
	/* Check the parameters */
...
/**
  ...
  *         FLASH::ERROR_WRP, FLASH::COMPLETE or FLASH::TIMEOUT.
  */
FLASH::Status FLASH::UserOptionByteConfig(uint16_t OB_IWDG, uint16_t OB_STOP, uint16_t OB_STDBY)
{
	FLASH::Status status = FLASH::COMPLETE;
...
/**
  ...
  *         FLASH::ERROR_WRP, FLASH::COMPLETE or FLASH::TIMEOUT.
  */
FLASH::Status FLASH::BootConfig(uint16_t FLASH_BOOT)
{
	FLASH::Status status = FLASH::COMPLETE;
...
/**
  ...
  *         and RST_STDBY(Bit2).
  */
uint32_t FLASH::GetUserOptionByte(void)
{
	/* Return the User Option Byte */
	return (uint32_t)(_FLASH->OBR >> 2);
}
...
/**
  ...
  */
uint32_t FLASH::GetWriteProtectionOptionByte(void)
{
	/* Return the Flash write protection Register value */
	return (uint32_t)(_FLASH->WRPR);
}
...
/**
  ...
  */
FlagStatus FLASH::GetReadOutProtectionStatus(void)
{
	FlagStatus readoutstatus = RESET;
	if((_FLASH->OBR & RDPRT_Mask) != (uint32_t)RESET)
	{
		readoutstatus = SET;
	}
	else
...
/**
  ...
  */
FlagStatus FLASH::GetPrefetchBufferStatus(void)
{
	FlagStatus bitstatus = RESET;

	if((_FLASH->ACR & ACR_PRFTBS_Mask) != (uint32_t)RESET)
	{
		bitstatus = SET;
	}
	else
...
/**
  ...
  * @retval None
  */
void FLASH::ITConfig(uint32_t FLASH_IT, FunctionalState NewState)
{
#ifdef STM32F10X_XL
	/* Check the parameters */
	...
	if((FLASH_IT & 0x80000000) != 0x0)
	{
		if(NewState != DISABLE)
		{
			...
			_FLASH->CR2 |= (FLASH_IT & 0x7FFFFFFF);
		}
		else
		{
			/* Disable the interrupt sources */
			_FLASH->CR2 &= ~(uint32_t)(FLASH_IT & 0x7FFFFFFF);
		}
	}
	else
	{
...
/**
  ...
  */
FlagStatus FLASH::GetFlagStatus(uint32_t FLASH_FLAG)
{
	FlagStatus bitstatus = RESET;
	...
	{
		if((_FLASH->OBR & FLASH_FLAG_OPTERR) != (uint32_t)RESET)
		{
			bitstatus = SET;
		}
		else
	...
	else
	{
		...
		{
			if((_FLASH->SR2 & FLASH_FLAG) != (uint32_t)RESET)
			{
				bitstatus = SET;
			}
			else
		...
		{
			if((_FLASH->SR & FLASH_FLAG) != (uint32_t)RESET)
			{
				bitstatus = SET;
			}
			else
...
/**
  ...
  * @retval None
  */
void FLASH::ClearFlag(uint32_t FLASH_FLAG)
{
#ifdef STM32F10X_XL
	/* Check the parameters */
	...
	{
		/* Clear the flags */
		_FLASH->SR2 = FLASH_FLAG;
	}
	else
	{
		/* Clear the flags */
		_FLASH->SR = FLASH_FLAG;
	}
...
/**
  ...
  * @param  None
  * @retval FLASH Status: The returned value can be: FLASH::BUSY, FLASH::ERROR_PG,
  *         FLASH::ERROR_WRP or FLASH::COMPLETE
  */
FLASH::Status FLASH::GetStatus(void)
{
	FLASH::Status flashstatus = FLASH::COMPLETE;

	if((_FLASH->SR & FLASH_FLAG_BSY) == FLASH_FLAG_BSY)
	{
		flashstatus = FLASH::BUSY;
	}
	else
	{
		if((_FLASH->SR & FLASH_FLAG_PGERR) != 0)
		{
			flashstatus = FLASH::ERROR_PG;
		}
		else
		{
			if((_FLASH->SR & FLASH_FLAG_WRPRTERR) != 0)
			{
				flashstatus = FLASH::ERROR_WRP;
			}
			else
			{
...
/**
  ...
  *         FLASH::ERROR_WRP or FLASH::COMPLETE
  */
FLASH::Status FLASH::GetBank1Status(void)
{
	FLASH::Status flashstatus = FLASH::COMPLETE;

	if((_FLASH->SR & FLASH_FLAG_BANK1_BSY) == FLASH_FLAG_BSY)
	{
		flashstatus = FLASH::BUSY;
	}
	else
	{
		if((_FLASH->SR & FLASH_FLAG_BANK1_PGERR) != 0)
		{
			flashstatus = FLASH::ERROR_PG;
		}
		else
		{
			if((_FLASH->SR & FLASH_FLAG_BANK1_WRPRTERR) != 0)
			{
				flashstatus = FLASH::ERROR_WRP;
			}
			else
...
/**
  ...
  *        FLASH::ERROR_WRP or FLASH::COMPLETE
  */
FLASH::Status FLASH::GetBank2Status(void)
{
	FLASH::Status flashstatus = FLASH::COMPLETE;

	if((_FLASH->SR2 & (FLASH_FLAG_BANK2_BSY & 0x7FFFFFFF)) == (FLASH_FLAG_BANK2_BSY & 0x7FFFFFFF))
	{
		flashstatus = FLASH::BUSY;
	}
	else
	{
		if((_FLASH->SR2 & (FLASH_FLAG_BANK2_PGERR & 0x7FFFFFFF)) != 0)
		{
			flashstatus = FLASH::ERROR_PG;
		}
		else
		{
			if((_FLASH->SR2 & (FLASH_FLAG_BANK2_WRPRTERR & 0x7FFFFFFF)) != 0)
			{
				flashstatus = FLASH::ERROR_WRP;
			}
			else
...
/**
  ...
  * @note   This function can be used for all STM32F10x devices,
  *         it is equivalent to WaitForLastBank1Operation.
  ...
  *         FLASH::ERROR_WRP, FLASH::COMPLETE or FLASH::TIMEOUT.
  */
FLASH::Status FLASH::WaitForLastOperation(uint32_t Timeout)
{
	FLASH::Status status = FLASH::COMPLETE;
	...
	while((status == FLASH::BUSY) && (Timeout != 0x00))
	{
		status = GetBank1Status();
		Timeout--;
	}
...
/**
  ...
  * @note   This function can be used for all STM32F10x devices,
  *         it is equivalent to WaitForLastOperation.
  * @param  Timeout: FLASH programming Timeout
  * @retval FLASH Status: The returned value can be: FLASH::ERROR_PG,
  *         FLASH::ERROR_WRP, FLASH::COMPLETE or FLASH::TIMEOUT.
  */
FLASH::Status FLASH::WaitForLastBank1Operation(uint32_t Timeout)
{
	FLASH::Status status = FLASH::COMPLETE;
...
/**
  ...
  *         FLASH::ERROR_WRP, FLASH::COMPLETE or FLASH::TIMEOUT.
  */
FLASH::Status FLASH::WaitForLastBank2Operation(uint32_t Timeout)
{
	FLASH::Status status = FLASH::COMPLETE;
