...
CEC::CEC(InitTypeDef& InitStruct)
{
	ObjectNum++;
	this->Init(InitStruct);
	this->On();
}
CEC::CEC(uint16_t BitTimingMode, uint16_t BitPeriodMode)
{
	CEC::InitTypeDef aaa;
	ObjectNum++;
	aaa.BitPeriodMode = BitPeriodMode;
	aaa.BitTimingMode = BitTimingMode;
	this->Init(aaa);
	this->On();
}
CEC_TypeDef* CEC::GetPeriphPointer(void)
{
	return _CEC;
}
void CEC::On()
...
/**
  ...
  * @retval None
  */
void CEC::DeInit(void)
{
	/* Enable CEC reset state */
	RCC::APB1PeriphResetCmd(RCC_APB1Periph_CEC, ENABLE);
	/* Release CEC from reset state */
	RCC::APB1PeriphResetCmd(RCC_APB1Periph_CEC, DISABLE);
}
...
/**
  ...
  * @retval None
  */
void CEC::Init(InitTypeDef &InitStruct)
{
	uint16_t tmpreg = 0;

	/* Check the parameters */
	assert_param(IS_CEC_BIT_TIMING_ERROR_MODE(InitStruct.BitTimingMode));
	assert_param(IS_CEC_BIT_PERIOD_ERROR_MODE(InitStruct.BitPeriodMode));
	RCC::APB1PeriphClockCmd(RCC_APB1Periph_CEC,ENABLE);
	...
	tmpreg |= (uint16_t)(InitStruct.BitTimingMode | InitStruct.BitPeriodMode);
...
/**
  ...
  * @retval None
  */
void CEC::Cmd(FunctionalState NewState)
{
	/* Check the parameters */
	...
	*(__IO uint32_t *) CFGR_PE_BB = (uint32_t)NewState;
	...
	if(NewState == DISABLE)
	{
		...
		while((_CEC->CFGR & CEC_CFGR_PE) != (uint32_t)RESET)
		{
		}
	}
}
...
/**
  ...
  * @retval None
  */
void CEC::ITConfig(FunctionalState NewState)
{
	/* Check the parameters */
	assert_param(IS_FUNCTIONAL_STATE(NewState));
...
/**
  ...
  * @retval None
  */
void CEC::OwnAddressConfig(uint8_t CEC_OwnAddress)
{
	/* Check the parameters */
	assert_param(IS_CEC_ADDRESS(CEC_OwnAddress));

	/* Set the CEC own address */
	_CEC->OAR = CEC_OwnAddress;
}
...
/**
  ...
  * @retval None
  */
void CEC::SetPrescaler(uint16_t CEC_Prescaler)
{
	/* Check the parameters */
	assert_param(IS_CEC_PRESCALER(CEC_Prescaler));

	/* Set the  Prescaler value*/
	_CEC->PRES = CEC_Prescaler;
}
...
/**
  ...
  * @retval None
  */
void CEC::SendDataByte(uint8_t Data)
{
	/* Transmit Data */
	_CEC->TXD = Data ;
}
...
/**
  ...
  */
uint8_t CEC::ReceiveDataByte(void)
{
	/* Receive Data */
	return (uint8_t)(_CEC->RXD);
}
...
/**
  ...
  * @retval None
  */
void CEC::EndOfMessageCmd(FunctionalState NewState)
{
	/* Check the parameters */
	assert_param(IS_FUNCTIONAL_STATE(NewState));
...
/**
  ...
  */
FlagStatus CEC::GetFlagStatus(uint32_t CEC_FLAG)
{
...
/**
  ...
  * @retval None
  */
void CEC::ClearFlag(uint32_t CEC_FLAG)
{
	uint32_t tmp = 0x0;
	...
	_CEC->CSR &= (uint32_t)(((~(uint32_t)CEC_FLAG) & 0xFFFFFFFC) | tmp);
}
...
/**
  ...
  */
ITStatus CEC::GetITStatus(uint8_t CEC_IT)
{
	...
	enablestatus = (_CEC->CFGR & (uint8_t)CEC_CFGR_IE) ;

	/* Check the status of the specified CEC interrupt */
	if(((_CEC->CSR & CEC_IT) != (uint32_t)RESET) && enablestatus)
	{
		/* CEC_IT is set */
		bitstatus = SET;
	}
...
/**
  ...
  * @retval None
  */
void CEC::ClearITPendingBit(uint16_t CEC_IT)
{
	uint32_t tmp = 0x0;
	...
	_CEC->CSR &= (uint32_t)(((~(uint32_t)CEC_IT) & 0xFFFFFFFC) | tmp);
}
