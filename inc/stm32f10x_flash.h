...
class FLASH
{
	public:
		...
		*/
		...
		FLASH_TypeDef* GetPeriphPointer(void);//获取内部外设指针
		/*------------ Functions used for all STM32F10x devices -----*/
		static void SetLatency(uint32_t FLASH_Latency);
		static void HalfCycleAccessCmd(uint32_t FLASH_HalfCycleAccess);
		static void PrefetchBufferCmd(uint32_t FLASH_PrefetchBuffer);
		static void Unlock(void);
		static void Lock(void);
		static Status ErasePage(uint32_t Page_Address);
		static Status EraseAllPages(void);
		static Status EraseOptionBytes(void);
		static Status ProgramWord(uint32_t Address, uint32_t Data);
		static Status ProgramHalfWord(uint32_t Address, uint16_t Data);
		static Status ProgramOptionByteData(uint32_t Address, uint8_t Data);
		static Status EnableWriteProtection(uint32_t FLASH_Pages);
		static Status ReadOutProtection(FunctionalState NewState);
		static Status UserOptionByteConfig(uint16_t OB_IWDG, uint16_t OB_STOP, uint16_t OB_STDBY);
		static uint32_t  GetUserOptionByte(void);
		static uint32_t  GetWriteProtectionOptionByte(void);
		static FlagStatus GetReadOutProtectionStatus(void);
		static FlagStatus GetPrefetchBufferStatus(void);
		static void ITConfig(uint32_t FLASH_IT, FunctionalState NewState);
		static FlagStatus GetFlagStatus(uint32_t FLASH_FLAG);
		static void ClearFlag(uint32_t FLASH_FLAG);
		static Status GetStatus(void);
		static Status WaitForLastOperation(uint32_t Timeout);

		/*------------ New function used for all STM32F10x devices -----*/
		static void UnlockBank1(void);
		static void LockBank1(void);
		static Status EraseAllBank1Pages(void);
		static Status GetBank1Status(void);
		static Status WaitForLastBank1Operation(uint32_t Timeout);

#ifdef STM32F10X_XL
		/*---- New Functions used only with STM32F10x_XL density devices -----*/
		static void UnlockBank2(void);
		static void LockBank2(void);
		static Status EraseAllBank2Pages(void);
		static Status GetBank2Status(void);
		static Status WaitForLastBank2Operation(uint32_t Timeout);
		static Status BootConfig(uint16_t FLASH_BOOT);
#endif
};
//#ifdef __cplusplus
//}
