...
typedef struct
{
	uint16_t BitTimingMode; /*!< Configures the CEC Bit Timing Error Mode.
                               This parameter can be a value of @ref CEC_BitTiming_Mode */
	uint16_t BitPeriodMode; /*!< Configures the CEC Bit Period Error Mode.
...
class CEC
{
	...
	public:
		friend class RCC;
		typedef _CEC_InitTypeDef InitTypeDef;
		CEC();
		CEC(InitTypeDef& InitStruct);
		CEC(uint16_t BitTimingMode, uint16_t BitPeriodMode);

		CEC_TypeDef* GetPeriphPointer(void);//获取内部外设指针
		void On();
		void Off();
		CEC& operator << (uint8_t Data);
		CEC& operator >> (uint8_t& Data);
		~CEC();
		...
		static void Init(InitTypeDef &InitStruct);
		static void Cmd(FunctionalState NewState);
		static void ITConfig(FunctionalState NewState);
		static void OwnAddressConfig(uint8_t CEC_OwnAddress);
		static void SetPrescaler(uint16_t CEC_Prescaler);
		static void SendDataByte(uint8_t Data);
		static uint8_t ReceiveDataByte(void);
		static void StartOfMessage(void);
		static void EndOfMessageCmd(FunctionalState NewState);
		static FlagStatus GetFlagStatus(uint32_t CEC_FLAG);
		static void ClearFlag(uint32_t CEC_FLAG);
		static ITStatus GetITStatus(uint8_t CEC_IT);
		static void ClearITPendingBit(uint16_t CEC_IT);
};
//#ifdef __cplusplus
//}
//#endif
