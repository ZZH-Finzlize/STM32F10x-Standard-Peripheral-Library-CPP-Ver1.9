...
class CAN
{
	...
	public:
		friend class RCC;
		typedef _CAN_InitTypeDef InitTypeDef;
		typedef _CAN_FilterInitTypeDef FilterInitTypeDef;
		typedef _CanTxMsg TxMsg;
		typedef _CanRxMsg RxMsg;
		CAN(CAN_TypeDef* _CANx);
		CAN(CAN_TypeDef* _CANx, InitTypeDef& InitStruct);
		CAN(CAN_TypeDef* _CANx, uint16_t Prescaler, uint8_t Mode, uint8_t SJW, uint8_t BS1, uint8_t BS2, \
		    FunctionalState TTCM, FunctionalState ABOM, FunctionalState AWUM, \
		    FunctionalState NART, FunctionalState RFLM, FunctionalState TXFP);
		CAN(const CAN& Other);

		~CAN();
		CAN& operator << (TxMsg& TxMessage);
		CAN_TypeDef* GetPeriphPointer(void);
